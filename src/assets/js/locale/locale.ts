...
export const sysGetLocaleResourceListByCompanyIdAndLocale = async (companyId: any, locale: string) => {
  const BASE_URL = 'sys/locale-resource/';
  try {
    let res: any = await Http.get(`${BASE_URL}sys-get-locale-resource-list-by-company-id-and-locale`, {
      companyId,
      locale,
